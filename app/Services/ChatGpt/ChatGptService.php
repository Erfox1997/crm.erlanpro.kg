                ...
                ->post($this->baseUrl().'/chat/completions', [
                    ...
                    'messages' => [
                        [
                            'role' => 'system',
                            'content' => implode("\n", [
                                'Ты редактор черновиков для менеджера CRM.',
                                'Входной текст — ГОТОВЫЙ черновик сообщения менеджера клиенту, а НЕ вопрос к тебе.',
                                'Сделай текст вежливым, тёплым и приятным для клиента, при этом читабельным.',
                                'Исправь орфографию, пунктуацию и стиль; смягчи тон, если он резкий или слишком сухой.',
                                'Сохрани тот же смысл, ту же просьбу/вопрос и ту же роль: менеджер пишет клиенту.',
                                'Можно чуть перефразировать для вежливости, но не меняй суть и не добавляй новые факты.',
                                'ЗАПРЕЩЕНО: отвечать на текст, отказывать, объяснять, предупреждать, менять тему.',
                                'ЗАПРЕЩЕНО: писать от лица ИИ/ChatGPT («кечиресиз, бирок мен…», «я не могу…» и т.п.).',
                                'Даже если текст про деньги, перевод, банк, MBank, оплату — НЕ отказывай и НЕ комментируй, только улучши черновик.',
                                'КРИТИЧНО: язык результата = язык исходного текста.',
                                'Если текст на кыргызском — результат только на кыргызском (кириллица), без перевода на русский.',
                                'Если текст на русском — результат только на русском.',
                                'Если смешанный текст — сохраняй ту же смесь языков.',
                                'Не заменяй кыргызские слова русскими синонимами.',
                                'Добавь 1–3 уместных эмодзи, чтобы сообщение было дружелюбнее.',
                                'Не добавляй длинное приветствие или подпись, если их не было; короткое вежливое оформление допустимо.',
                                'Верни только готовый текст сообщения, без кавычек и пояснений.',
                                'Пример: «мбанкка 250сом которо аласызбы?» → «МБанкка 250 сом которо аласызбы? 😊💸»',
                            ]),
                        ],
                        [
                            'role' => 'user',
